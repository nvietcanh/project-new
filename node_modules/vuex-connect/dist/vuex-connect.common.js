...
var LIFECYCLE_KEYS = ['beforeCreate', 'created', 'beforeMount', 'mounted', 'beforeUpdate', 'updated', 'activated', 'deactivated', 'beforeDestroy', 'destroyed'];
...
var createConnect = function createConnect(transform) {
  return function () {
    ...
    return function (name, Component) {
      ...
      var originalProps = getOptions(Component).props || {};

      var containerProps = void 0,
          containerPropsKeys = void 0;
      if (Array.isArray(originalProps)) {
        containerProps = containerPropsKeys = originalProps.filter(function (p) {
          return propKeys.indexOf(p) < 0;
        });
      } else {
...
function insertRenderer(options, name, propKeys, eventKeys) {
  options.render = function (h) {
    ...
    }, flattenObject(this.$slots).map(function (slot) {
      return patchSlot(slot, _this);
    }));
  };
}
